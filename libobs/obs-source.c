...
#include "media/audio-io.h"
#include "media/format-conversion.h"
#include "media/video-frame.h"
...
bool obs_source_init_context(struct obs_source* source, obs_data_t* settings, const char* name, const char* uuid, obs_data_t* hotkey_data,
                             bool private)
{
...
static void obs_source_init_audio_hotkeys(struct obs_source* source)
{
    ...
    source->mute_unmute_key =
        obs_hotkey_pair_register_source(source, "libobs.mute", obs->hotkeys.mute, "libobs.unmute", obs->hotkeys.unmute,
                                        obs_source_hotkey_mute, obs_source_hotkey_unmute, source, source);

    source->push_to_mute_key =
        obs_hotkey_register_source(source, "libobs.push-to-mute", obs->hotkeys.push_to_mute, obs_source_hotkey_push_to_mute, source);

    source->push_to_talk_key =
        obs_hotkey_register_source(source, "libobs.push-to-talk", obs->hotkeys.push_to_talk, obs_source_hotkey_push_to_talk, source);
}

static obs_source_t* obs_source_create_internal(const char* id, const char* name, const char* uuid, obs_data_t* settings,
                                                obs_data_t* hotkey_data, bool private, uint32_t last_obs_ver)
{
...
obs_source_t* obs_source_duplicate(obs_source_t* source, const char* new_name, bool create_private)
{
    ...
    if (source->info.type == OBS_SOURCE_TYPE_SCENE)
    {
        ...
        obs_scene_t* new_scene = obs_scene_duplicate(scene, new_name, create_private ? OBS_SCENE_DUP_PRIVATE_COPY : OBS_SCENE_DUP_COPY);
...
static void obs_source_destroy_defer(struct obs_source* source)
{
    ...
    blog(LOG_DEBUG, "%ssource '%s' destroyed", source->context.is_private ? "private " : "", source->context.name);
...
{
    return data_valid(source, "obs_source_configurable") && (source->info.get_properties || source->info.get_properties2);
}
...
static void source_output_audio_data(obs_source_t* source, const struct audio_data* data)
{
    ...
    else if (source->next_audio_ts_min != 0)
    {
        ...
        {
            blog(LOG_DEBUG,
                 "Audio timestamp for '%s' exceeded TS_SMOOTHING_THRESHOLD, diff=%" PRIu64 " ns, expected %" PRIu64 ", input %" PRIu64,
...
bool set_async_texture_size(struct obs_source* source, const struct obs_source_frame* frame)
{
    ...
    if (source->async_width == frame->width && source->async_height == frame->height && source->async_format == frame->format
        && source->async_full_range == frame->full_range && source->async_trc == frame->trc)
        return true;
    ...
    if (async_gpu_conversion)
    {
        ...
        for (int c = 0; c < source->async_channel_count; ++c)
            source->async_textures[c] = gs_texture_create(source->async_convert_width[c], source->async_convert_height[c],
...
{
    return (format == VIDEO_FORMAT_I010) || (format == VIDEO_FORMAT_P010) || (format == VIDEO_FORMAT_I210) || (format == VIDEO_FORMAT_I412)
...
static bool update_async_texrender(struct obs_source* source, const struct obs_source_frame* frame, gs_texture_t* tex[MAX_AV_PLANES],
...
bool update_async_textures(struct obs_source* source, const struct obs_source_frame* frame, gs_texture_t* tex[MAX_AV_PLANES],
                           gs_texrender_t* texrender)
{
...
enum gs_color_space obs_source_get_color_space(obs_source_t* source, size_t count, const enum gs_color_space* preferred_spaces)
{
    ...
    return source->info.video_get_color_space ? source->info.video_get_color_space(source->context.data, count, preferred_spaces)
                                              : GS_CS_SRGB;
}
...
void obs_source_filter_add(obs_source_t* source, obs_source_t* filter)
{
    ...
    blog(LOG_DEBUG, "- filter '%s' (%s) added to source '%s'", filter->context.name, filter->info.id, source->context.name);
...
static bool obs_source_filter_remove_refless(obs_source_t* source, obs_source_t* filter)
{
    ...
    blog(LOG_DEBUG, "- filter '%s' (%s) removed from source '%s'", filter->context.name, filter->info.id, source->context.name);
...
static inline void copy_frame_data_line(struct obs_source_frame* dst, const struct obs_source_frame* src, uint32_t plane, uint32_t y)
{
...
static inline void copy_frame_data_plane(struct obs_source_frame* dst, const struct obs_source_frame* src, uint32_t plane, uint32_t lines)
{
...
static inline void reset_resampler(obs_source_t* source, const struct obs_source_audio* audio)
{
    ...
    if (source->sample_info.samples_per_sec == obs_info->samples_per_sec && source->sample_info.format == obs_info->format
...
static void process_audio_balancing(struct obs_source* source, uint32_t frames, float balance, enum obs_balance_type type)
{
    float** data = (float**)source->audio_data.data;
...
static void process_audio(obs_source_t* source, const struct obs_source_audio* audio)
{
    ...
    if (source->sample_info.samples_per_sec != audio->samples_per_sec || source->sample_info.format != audio->format
        || source->sample_info.speakers != audio->speakers)
    ...
    if (source->resampler)
    {
        ...
        audio_resampler_resample(source->resampler, output, &frames, &source->resample_offset, audio->data, audio->frames);
    ...
    if (!mono_output && source->sample_info.speakers == SPEAKERS_STEREO && (source->balance > 0.51f || source->balance < 0.49f))
    {
...
static bool ready_async_frame(obs_source_t* source, uint64_t sys_time)
{
    ...
    blog(LOG_DEBUG,
         ...
         "number of frames: %lu",
         source->last_frame_ts, frame_time, sys_offset, frame_time - source->last_frame_ts, (unsigned long)source->async_frames.num);
#endif
...
static inline void render_filter_tex(gs_texture_t* tex, gs_effect_t* effect, uint32_t width, uint32_t height, const char* tech_name)
{
    gs_technique_t* tech = gs_effect_get_technique(effect, tech_name);
...
{
    return (target == parent) && (allow_direct == OBS_ALLOW_DIRECT_RENDERING) && ((parent_flags & OBS_SOURCE_CUSTOM_DRAW) == 0)
...
bool obs_source_process_filter_begin_with_color_space(obs_source_t* filter, enum gs_color_format format, enum gs_color_space space,
...
void obs_source_enable_push_to_mute(obs_source_t* source, bool enabled)
{
    ...
    if (obs_source_get_output_flags(source) & OBS_SOURCE_AUDIO && changed)
        blog(LOG_INFO, "source '%s' %s push-to-mute", obs_source_get_name(source), enabled ? "enabled" : "disabled");
...
void obs_source_enable_push_to_talk(obs_source_t* source, bool enabled)
{
    ...
    if (obs_source_get_output_flags(source) & OBS_SOURCE_AUDIO && changed)
        blog(LOG_INFO, "source '%s' %s push-to-talk", obs_source_get_name(source), enabled ? "enabled" : "disabled");
...
{
    return obs_source_valid(source, "obs_source_get_monitoring_type") ? source->monitoring_type : OBS_MONITORING_TYPE_NONE;
}
