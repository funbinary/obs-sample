...
//    if (video->using_nv12_tex)
//    {
//        if (!gs_texture_create_nv12(&video->convert_textures_encode[0], &video->convert_textures_encode[1], info->width, info->height,
...
//    {
//        if (!gs_texture_create_p010(&video->convert_textures_encode[0], &video->convert_textures_encode[1], info->width, info->height,
//                                    GS_RENDER_TARGET | GS_SHARED_KM_TEX))
//        {
//            return false;
//        }
...
//    {
//    case VIDEO_FORMAT_I420:
//        video->convert_textures[0] = gs_texture_create(info->width, info->height, GS_R8, 1, NULL, GS_RENDER_TARGET);
//        video->convert_textures[1] = gs_texture_create(info->width / 2, info->height / 2, GS_R8, 1, NULL, GS_RENDER_TARGET);
//        video->convert_textures[2] = gs_texture_create(info->width / 2, info->height / 2, GS_R8, 1, NULL, GS_RENDER_TARGET);
//        if (!video->convert_textures[0] || !video->convert_textures[1] || !video->convert_textures[2])
//            success = false;
//        break;
//    case VIDEO_FORMAT_NV12:
//        video->convert_textures[0] = gs_texture_create(info->width, info->height, GS_R8, 1, NULL, GS_RENDER_TARGET);
//        video->convert_textures[1] = gs_texture_create(info->width / 2, info->height / 2, GS_R8G8, 1, NULL, GS_RENDER_TARGET);
//        if (!video->convert_textures[0] || !video->convert_textures[1])
//            success = false;
//        break;
//    case VIDEO_FORMAT_I444:
//        video->convert_textures[0] = gs_texture_create(info->width, info->height, GS_R8, 1, NULL, GS_RENDER_TARGET);
//        video->convert_textures[1] = gs_texture_create(info->width, info->height, GS_R8, 1, NULL, GS_RENDER_TARGET);
//        video->convert_textures[2] = gs_texture_create(info->width, info->height, GS_R8, 1, NULL, GS_RENDER_TARGET);
//        if (!video->convert_textures[0] || !video->convert_textures[1] || !video->convert_textures[2])
//            success = false;
//        break;
//    case VIDEO_FORMAT_I010:
//        video->convert_textures[0] = gs_texture_create(info->width, info->height, GS_R16, 1, NULL, GS_RENDER_TARGET);
//        video->convert_textures[1] = gs_texture_create(info->width / 2, info->height / 2, GS_R16, 1, NULL, GS_RENDER_TARGET);
//        video->convert_textures[2] = gs_texture_create(info->width / 2, info->height / 2, GS_R16, 1, NULL, GS_RENDER_TARGET);
//        if (!video->convert_textures[0] || !video->convert_textures[1] || !video->convert_textures[2])
//            success = false;
//        break;
//    case VIDEO_FORMAT_P010:
//        video->convert_textures[0] = gs_texture_create(info->width, info->height, GS_R16, 1, NULL, GS_RENDER_TARGET);
//        video->convert_textures[1] = gs_texture_create(info->width / 2, info->height / 2, GS_RG16, 1, NULL, GS_RENDER_TARGET);
//        if (!video->convert_textures[0] || !video->convert_textures[1])
//            success = false;
//        break;
//    case VIDEO_FORMAT_P216:
//        video->convert_textures[0] = gs_texture_create(info->width, info->height, GS_R16, 1, NULL, GS_RENDER_TARGET);
//        video->convert_textures[1] = gs_texture_create(info->width / 2, info->height, GS_RG16, 1, NULL, GS_RENDER_TARGET);
//        if (!video->convert_textures[0] || !video->convert_textures[1])
//            success = false;
//        break;
//    case VIDEO_FORMAT_P416:
//        video->convert_textures[0] = gs_texture_create(info->width, info->height, GS_R16, 1, NULL, GS_RENDER_TARGET);
//        video->convert_textures[1] = gs_texture_create(info->width, info->height, GS_RG16, 1, NULL, GS_RENDER_TARGET);
//        if (!video->convert_textures[0] || !video->convert_textures[1])
//            success = false;
//        break;
//    default:
...
//    struct gs_sampler_info point_sampler = {static_cast<gs_sample_filter>(0)};
//    bool success = true;
//    int errorcode;
//
...
//    {
//        video_format_get_parameters_for_format(info->colorspace, info->range, info->format, (float*)&mat, NULL, NULL);
//        matrix4_inv(&mat, &mat);
//
//        /* swap R and G */
//        r_row = mat.x;
...
//{
//    struct obs_core_video_mix* video = static_cast<obs_core_video_mix*>(bzalloc(sizeof(struct obs_core_video_mix)));
...
//{
//    obs = static_cast<obs_core*>(bzalloc(sizeof(struct obs_core)));
//
...
//        len += strlen(argv[i]) + 1;
//
//    cmdline_args.argv = static_cast<char**>(bmalloc(sizeof(char*) * (argc + 1) + len));
...
//         "\tformat:            %s\n"
//         "\tYUV mode:          %s%s%s",
//         ovi->base_width, ovi->base_height, ovi->output_width, ovi->output_height, scale_type_name, ovi->fps_num, ovi->fps_den,
//         get_video_format_name(ovi->output_format), yuv ? yuv_format : "None", yuv ? "/" : "", yuv ? yuv_range : "");
//
...
//
//    int max_buffering_ms = audio->max_buffering_ticks * AUDIO_OUTPUT_FRAMES * SEC_TO_MSEC / (int)oai->samples_per_sec;
//
//    ai.name = "Audio";
...
//         "\tbuffering type:  %s",
//         (int)ai.samples_per_sec, (int)ai.speakers, max_buffering_ms, oai->fixed_buffering ? "fixed" : "dynamically increasing");
//
...
//
//        if (strcmp(callback->id, id) == 0 && strcmp(callback->task, task) == 0 && strcmp(callback->target, target) == 0)
//            return callback;
//    }
//
//    return NULL;
...
//{
//    struct obs_context_data **start = static_cast<obs_context_data**>(pstart), *context;
//    using ProcFunc = bool (*)(void*, void*);
//    ProcFunc enum_proc;
//    enum_proc = reinterpret_cast<ProcFunc>(proc);
//
//    assert(start);
//    assert(mutex);
//    assert(enum_proc);
...
//{
//    struct obs_context_data **start = static_cast<obs_context_data**>(pstart), *context, *tmp;
//    using ProcFunc = bool (*)(void*, void*);
//    ProcFunc enum_proc;
//    enum_proc = reinterpret_cast<ProcFunc>(proc);
//
...
//{
//    obs_enum_uuid(&obs->data.sources, &obs->data.sources_mutex, (void*)enum_proc, param);
//}
//
//void obs_enum_outputs(bool (*enum_proc)(void*, obs_output_t*), void* param)
//{
//    obs_enum(&obs->data.first_output, &obs->data.outputs_mutex, (void*)enum_proc, param);
//}
//
//void obs_enum_encoders(bool (*enum_proc)(void*, obs_encoder_t*), void* param)
//{
//    obs_enum(&obs->data.first_encoder, &obs->data.encoders_mutex, (void*)enum_proc, param);
//}
//
//void obs_enum_services(void (*enum_proc)(void*, obs_service_t*), void* param)
//{
//    obs_enum(&obs->data.first_service, &obs->data.services_mutex, (void*)enum_proc, param);
//}
//
//static inline void* get_context_by_name(void* vfirst, const char* name, pthread_mutex_t* mutex, void* (*addref)(void*))
//{
//    struct obs_context_data** first = static_cast<obs_context_data**>(vfirst);
...
//{
//    struct obs_context_data** ht = static_cast<obs_context_data**>(ptable);
//    struct obs_context_data* context;
//
//    pthread_mutex_lock(mutex);
//
...
//{
//    return (obs_source_t*)obs_source_get_ref((obs_source_t*)ref);
//}
//
//static inline void* obs_output_addref_safe_(void* ref)
//{
//    return (obs_output_t*)obs_output_get_ref((obs_output_t*)ref);
//}
//
//static inline void* obs_encoder_addref_safe_(void* ref)
//{
//    return static_cast<void*>(obs_encoder_get_ref((obs_encoder_t*)ref));
//}
//
//static inline void* obs_service_addref_safe_(void* ref)
//{
//    return static_cast<void*>(obs_service_get_ref((obs_service_t*)ref));
//}
//
...
//{
//    return static_cast<obs_source_t*>(
...
//{
//    return static_cast<obs_source_t*>(get_context_by_uuid(&obs->data.sources, uuid, &obs->data.sources_mutex, obs_source_addref_safe_));
//}
//
...
//        {
//            source = static_cast<obs_source*>(obs_source_addref_safe_(source));
//            break;
//        }
//        source = static_cast<obs_source*>((void*)source->context.hh_uuid.next);
//    }
//
//    pthread_mutex_unlock(&obs->data.sources_mutex);
//    return source;
...
//{
//    return static_cast<obs_output_t*>(
//        get_context_by_name(&obs->data.first_output, name, &obs->data.outputs_mutex, obs_output_addref_safe_));
//}
//
//obs_encoder_t* obs_get_encoder_by_name(const char* name)
//{
//    return static_cast<obs_encoder_t*>(
//        get_context_by_name(&obs->data.first_encoder, name, &obs->data.encoders_mutex, obs_encoder_addref_safe_));
//}
//
//obs_service_t* obs_get_service_by_name(const char* name)
//{
//    return static_cast<obs_service_t*>(
//        get_context_by_name(&obs->data.first_service, name, &obs->data.services_mutex, obs_service_addref_safe_));
//}
//
...
//}
//
//static void obs_render_main_texture_internal(enum gs_blend_type src_c, enum gs_blend_type dest_c, enum gs_blend_type src_a,
...
//    while (source)
//    {
//        if ((source->info.type != OBS_SOURCE_TYPE_FILTER) != 0 && !source->removed && !source->temp_removed && cb(data_, source))
//        {
...
//}
//
//static inline bool obs_context_data_init_wrap(struct obs_context_data* context, enum obs_obj_type type, obs_data_t* settings,
//                                              const char* name, const char* uuid, obs_data_t* hotkey_data, bool is_private)
//{
//    assert(context);
...
//}
//
//bool obs_context_data_init(struct obs_context_data* context, enum obs_obj_type type, obs_data_t* settings, const char* name,
//                           const char* uuid, obs_data_t* hotkey_data, bool is_private)
//{
//    if (obs_context_data_init_wrap(context, type, settings, name, uuid, hotkey_data, is_private))
//    {
//        return true;
//    }
//    else
...
//{
//    context->control = static_cast<obs_weak_object*>(bzalloc(sizeof(obs_weak_object_t)));
//    context->control->object = static_cast<obs_context_data*>(object);
//    context->destroy = destroy;
//}
//
//void obs_context_data_insert(struct obs_context_data* context, pthread_mutex_t* mutex, void* pfirst)
//{
//    struct obs_context_data** first = static_cast<obs_context_data**>(pfirst);
//
//    assert(context);
//    assert(mutex);
//    assert(first);
...
//{
//    struct obs_context_data* head = static_cast<obs_context_data*>(phash);
...
//{
//    struct obs_context_data** first_uuid = static_cast<obs_context_data**>(pfirst_uuid);
//    struct obs_context_data* item = NULL;
//
//    assert(context);
//    assert(mutex);
...
//{
//    struct obs_context_data** head = static_cast<obs_context_data**>(phead);
//
//    assert(head);
//
//    if (!context)
...
//{
//    struct obs_context_data** uuid_head = static_cast<obs_context_data**>(puuid_head);
//
//    assert(uuid_head);
//
...
//{
//    struct obs_context_data* context = static_cast<obs_context_data*>(obj);
...
//}
//
//void obs_add_raw_video_callback(const struct video_scale_info* conversion, void (*callback)(void* param, struct video_data* frame),
//                                void* param)
//{
...
//{
//    struct obs_core_video_mix* video = get_mix_for_video((video_t*)encoder->media);
//    bool success = true;
//
//    obs_enter_graphics();
...
//{
//    struct obs_core_video_mix* video = (obs_core_video_mix*)get_mix_for_video((video_t*)encoder->media);
...
//        struct obs_core_video_mix* video = obs->video.mixes.array[i];
//
//        if (os_atomic_load_long(&video->raw_active) > 0 || os_atomic_load_long(&video->gpu_encoder_active) > 0)
//        {
//            result = true;
//            break;
//        }
...
//{
//    struct task_wait_info* info = static_cast<task_wait_info*>(param);
//    if (info->task)
