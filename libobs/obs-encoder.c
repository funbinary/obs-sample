...
static inline bool gpu_encode_available(const struct obs_encoder* encoder)
{
    ...
    if (!video)
        return false;
    return (encoder->info.caps & OBS_ENCODER_CAP_PASS_TEXTURE) != 0 && (video->using_p010_tex || video->using_nv12_tex);
}
...
static inline bool obs_encoder_initialize_internal(obs_encoder_t* encoder)
{
    if (!encoder->media)
    {
        blog(LOG_ERROR, "obs_encoder_initialize_internal: encoder '%s' has no media set", encoder->context.name);
        return false;
    }
...
static inline size_t get_callback_idx(const struct obs_encoder* encoder, void (*new_packet)(void* param, struct encoder_packet* packet),
                                      void* param)
{
...
static inline void obs_encoder_start_internal(obs_encoder_t* encoder, void (*new_packet)(void* param, struct encoder_packet* packet),
                                              void* param)
{
...
static inline bool obs_encoder_stop_internal(obs_encoder_t* encoder, void (*new_packet)(void* param, struct encoder_packet* packet),
                                             void* param)
{
    bool last = false;
    size_t idx;
...
bool obs_encoder_set_frame_rate_divisor(obs_encoder_t* encoder, uint32_t frame_rate_divisor)
{
    ...
    if (encoder->media)
    {
        encoder->fps_override = video_output_create_with_frame_rate_divisor(encoder->media, encoder->frame_rate_divisor);
    }
...
static void encoder_set_video(obs_encoder_t* encoder, video_t* video)
{
    ...
    if (video)
    {
        ...
        {
            encoder->fps_override = video_output_create_with_frame_rate_divisor(video, encoder->frame_rate_divisor);
        }
    }
    else
    {
...
static bool send_audio_data(struct obs_encoder* encoder)
{
    ...
    for (size_t i = 0; i < encoder->planes; i++)
    {
        circlebuf_pop_front(&encoder->audio_input_buffer[i], encoder->audio_output_buffer[i], encoder->framesize_bytes);
