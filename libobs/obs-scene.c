...
static void calculate_bounds_data(struct obs_scene_item* item, struct vec2* origin, struct vec2* scale, uint32_t* cx, uint32_t* cy)
{
...
static inline bool item_texture_enabled(const struct obs_scene_item* item)
{
    return crop_enabled(&item->crop) || scale_filter_enabled(item) || (item->blend_method == OBS_BLEND_METHOD_SRGB_OFF)
           || !default_blending_enabled(item) || (item_is_scene(item) && !item->is_group);
}

static void render_item_texture(struct obs_scene_item* item, enum gs_color_space current_space, enum gs_color_space source_space)
{
    ...
    if (type != OBS_SCALE_DISABLE)
    {
        ...
        else if (!close_float(item->output_scale.x, 1.0f, EPSILON) || !close_float(item->output_scale.y, 1.0f, EPSILON))
        {
    ...
    gs_blend_function_separate(obs_blend_mode_params[item->blend_type].src_color, obs_blend_mode_params[item->blend_type].dst_color,
                               obs_blend_mode_params[item->blend_type].src_alpha, obs_blend_mode_params[item->blend_type].dst_alpha);
...
static bool scene_audio_render(void* data, uint64_t* ts_out, struct obs_source_audio_mix* audio_output, uint32_t mixers, size_t channels,
                               size_t sample_rate)
{
...
const struct obs_source_info scene_info = {
    ...
    .output_flags = OBS_SOURCE_VIDEO | OBS_SOURCE_CUSTOM_DRAW | OBS_SOURCE_COMPOSITE | OBS_SOURCE_DO_NOT_DUPLICATE | OBS_SOURCE_SRGB,
...
static inline obs_source_t* dup_child(obs_scene_item_ptr_array_t* old_items, size_t idx, obs_scene_t* new_scene, bool private)
{
    obs_source_t* source;
...
static inline void duplicate_item_data(struct obs_scene_item* dst, struct obs_scene_item* src, bool defer_texture_update,
                                       bool duplicate_hotkeys)
{
    ...
    {
        obs_source_t* transition = obs_source_duplicate(src->show_transition, obs_source_get_name(src->show_transition), true);
        obs_sceneitem_set_transition(dst, true, transition);
        obs_source_release(transition);
    }
    if (src->hide_transition)
    {
        obs_source_t* transition = obs_source_duplicate(src->hide_transition, obs_source_get_name(src->hide_transition), true);
...
obs_scene_t* obs_scene_duplicate(obs_scene_t* scene, const char* name, enum obs_scene_duplicate_type type)
{
    ...
    new_scene = make_private ? create_private_id(scene->source->info.id, name) : create_id(scene->source->info.id, name);
...
static void init_hotkeys(obs_scene_t* scene, obs_sceneitem_t* item, const char* name)
{
    ...
    item->toggle_visibility = obs_hotkey_pair_register_source(scene->source, show.array, show_desc.array, hide.array, hide_desc.array,
                                                              hotkey_show_sceneitem, hotkey_hide_sceneitem, item, item);
...
static void obs_sceneitem_destroy(obs_sceneitem_t* item)
{
    if (item)
    {
        ...
        signal_handler_disconnect(obs_source_get_signal_handler(item->source), "rename", sceneitem_renamed, item);
        if (item->show_transition)
...
{
    return crop1->left == crop2->left && crop1->right == crop2->right && crop1->top == crop2->top && crop1->bottom == crop2->bottom;
}
...
obs_sceneitem_t* obs_scene_insert_group(obs_scene_t* scene, const char* name, obs_sceneitem_t** items, size_t count)
{
    ...
    obs_sceneitem_t* item = obs_scene_add_internal(scene, sub_scene->source, last_item);
