...
#ifdef __APPLE__
    if (!dstr_find(&dylib_name, ".framework") && !dstr_find(&dylib_name, ".plugin") && !dstr_find(&dylib_name, ".dylib")
...
double os_cpu_usage_info_query(os_cpu_usage_info_t* info)
{
    ...
    if (cur_cpu_time <= info->last_cpu_time || time_sample.tms_stime < info->last_sys_time || time_sample.tms_utime < info->last_user_time)
        return 0.0;

    percent = (double)(time_sample.tms_stime - info->last_sys_time + (time_sample.tms_utime - info->last_user_time));
...
static void os_get_cores_internal(void)
{
    ...
    while (getline(&line, &linecap, fp) != -1)
    {
        ...
        if (!strncmp(line, "cpu cores", 9))
        {
            ...
            if (dstr_is_empty(&proc_phys_ids) || (!dstr_is_empty(&proc_phys_ids) && !dstr_find(&proc_phys_ids, proc_phys_id.array)))
            {
...
static inline bool os_get_proc_memory_usage_internal(statm_t* statm)
{
    ...
    if (fscanf(f, "%lu %lu %lu %lu %lu %lu %lu", &statm->virtual_size, &statm->resident_size, &statm->share_pages, &statm->text,
               &statm->library, &statm->data, &statm->dirty_pages)
        != 7)
    {
        fclose(f);
        return false;
