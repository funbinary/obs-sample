...
static void log_available_memory(void)
{
    ...
    blog(LOG_INFO, "Physical Memory: %luMB Total, %luMB Free%s", (DWORD)(ms.ullTotalPhys / 1048576), (DWORD)(ms.ullAvailPhys / 1048576),
         note);
}
...
static void log_windows_version(void)
{
    ...
    blog(LOG_INFO, "Windows Version: %d.%d Build %d (release: %s; revision: %d; %s%s-bit)", ver.major, ver.minor, ver.build, release_id,
         ver.revis, arm64_windows, windows_bitness);
}
...
static void log_admin_status(void)
{
    ...
    success = AllocateAndInitializeSid(&auth, 2, SECURITY_BUILTIN_DOMAIN_RID, DOMAIN_ALIAS_RID_ADMINS, 0, 0, 0, 0, 0, 0, &admin_group);
    if (success)
    {
...
static void log_gaming_features(void)
{
    ...
    {
        blog(LOG_INFO, "\tGame DVR Background Recording: %s", (bool)game_dvr_bg_recording.return_value ? "On" : "Off");
    }
...
void obs_key_to_str(obs_key_t key, struct dstr* str)
{
    ...
    if ((key < OBS_KEY_VK_CANCEL || key > OBS_KEY_VK_OEM_CLEAR) && scan_code != 0 && GetKeyNameTextW(scan_code, name, 128) != 0)
    {
