...
void obs_nix_x11_log_info(void)
{
    ...
    if (strstr(vendor_name, "X.Org"))
    {
        blog(LOG_INFO,
             ...
             ".%d.%d",
             protocol_version, protocol_revision, vendor_name, vendor_release / 10000000, (vendor_release / 100000) % 100,
...
static bool key_pressed(xcb_connection_t* connection, obs_hotkeys_platform_t* context, obs_key_t key)
{
    ...
    {
        pressed = keycode_pressed(reply, context->super_l_code) || keycode_pressed(reply, context->super_r_code);
    }
    else
    {
