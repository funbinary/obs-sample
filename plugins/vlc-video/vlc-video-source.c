...
{
    return (file && strstr(file, "://") != NULL) ? libvlc_media_new_location_(libvlc, file) : libvlc_media_new_path_(libvlc, file);
}
...
static unsigned vlcs_video_format(void** p_data, char* chroma, unsigned* width, unsigned* height, unsigned* pitches, unsigned* lines)
{
    ...
    if (c->frame.format != new_format || c->frame.width != *width || c->frame.height != *height)
    {
        ...
        video_format_get_parameters_for_format(VIDEO_CS_DEFAULT, range, new_format, c->frame.color_matrix, c->frame.color_range_min,
...
static int vlcs_audio_setup(void** p_data, char* format, unsigned* rate, unsigned* channels)
{
    ...
    if (c->audio.format == new_audio_format && c->audio.samples_per_sec == *rate && c->audio.speakers == (enum speaker_layout) * channels)
        return 0;
...
static void add_file(struct vlc_source* c, media_file_array_t* new_files, const char* path, int network_caching, int track_index,
...
static void* vlcs_create(obs_data_t* settings, obs_source_t* source)
{
    ...
    c->play_pause_hotkey =
        obs_hotkey_register_source(source, "VLCSource.PlayPause", obs_module_text("PlayPause"), vlcs_play_pause_hotkey, c);
    ...
    c->playlist_next_hotkey =
        obs_hotkey_register_source(source, "VLCSource.PlaylistNext", obs_module_text("PlaylistNext"), vlcs_playlist_next_hotkey, c);

    c->playlist_prev_hotkey =
        obs_hotkey_register_source(source, "VLCSource.PlaylistPrev", obs_module_text("PlaylistPrev"), vlcs_playlist_prev_hotkey, c);
        ...
        if (strcmp(path, "") != 0)
        {
            if (!os_file_exists(path) && strstr(path, "://") == NULL)
            {
                obs_missing_file_t* file =
                    obs_missing_file_create(path, missing_file_callback, OBS_MISSING_FILE_SOURCE, source, (void*)path);
