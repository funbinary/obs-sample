...
static void v4l2_prep_obs_frame(struct v4l2_data* data, struct obs_source_frame* frame, size_t* plane_offsets)
{
    ...
    frame->format = format;
    video_format_get_parameters_for_format(VIDEO_CS_DEFAULT, data->color_range, format, frame->color_matrix, frame->color_range_min,
...
static void* v4l2_thread(void* vptr)
{
    ...
    blog(LOG_INFO, "%s: select timeout set to %" PRIu64 " (%dx frame periods)", data->device_id, timeout_usec, data->timeout_frames);
    ...
    while (os_event_try(data->event) == EAGAIN)
    {
        ...
        blog(LOG_DEBUG, "%s: ts: %06ld buf id #%d, flags 0x%08X, seq #%d, len %d, used %d", data->device_id, buf.timestamp.tv_usec,
             buf.index, buf.flags, buf.sequence, buf.length, buf.bytesused);
...
static void v4l2_device_list(obs_property_t* prop, obs_data_t* settings)
{
    ...
    while ((dp = readdir(dirp)) != NULL)
    {
        ...
        int ret = snprintf(unique_device_name, sizeof(unique_device_name), "%s (%s)", video_cap.card, video_cap.bus_info);
...
static void v4l2_format_list(int dev, obs_property_t* prop)
{
    ...
    while (v4l2_ioctl(dev, VIDIOC_ENUM_FMT, &fmt) == 0)
    {
        ...
        if (v4l2_to_obs_video_format(fmt.pixelformat) != VIDEO_FORMAT_NONE || fmt.pixelformat == V4L2_PIX_FMT_MJPEG
...
static void v4l2_dv_timing_list(int dev, obs_property_t* prop)
{
    ...
    while (v4l2_enum_dv_timing(dev, &dvt, index) == 0)
    {
        ...
        double h = (double)dvt.bt.height + dvt.bt.vfrontporch + dvt.bt.vsync + dvt.bt.vbackporch + dvt.bt.il_vfrontporch + dvt.bt.il_vsync
...
static void v4l2_resolution_list(int dev, uint_fast32_t pixelformat, obs_property_t* prop)
{
    ...
    case V4L2_FRMSIZE_TYPE_DISCRETE:
        while (v4l2_ioctl(dev, VIDIOC_ENUM_FRAMESIZES, &frmsize) == 0)
        {
            ...
            obs_property_list_add_int(prop, buffer.array, v4l2_pack_tuple(frmsize.discrete.width, frmsize.discrete.height));
            frmsize.index++;
        }
        break;
    default:
 ...
 */
static void v4l2_framerate_list(int dev, uint_fast32_t pixelformat, uint_fast32_t width, uint_fast32_t height, obs_property_t* prop)
{
...
static obs_properties_t* v4l2_properties(void* vptr)
{
    ...
    obs_property_t* device_list =
        obs_properties_add_list(props, "device_id", obs_module_text("Device"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);

    obs_property_t* input_list =
        obs_properties_add_list(props, "input", obs_module_text("Input"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);

    obs_property_t* format_list =
        obs_properties_add_list(props, "pixelformat", obs_module_text("VideoFormat"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);

    obs_property_t* standard_list =
        obs_properties_add_list(props, "standard", obs_module_text("VideoStandard"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
    obs_property_set_visible(standard_list, false);

    obs_property_t* dv_timing_list =
        obs_properties_add_list(props, "dv_timing", obs_module_text("DVTiming"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
    ...
    obs_properties_add_list(props, "framerate", obs_module_text("FrameRate"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);

    obs_property_t* color_range_list =
        obs_properties_add_list(props, "color_range", obs_module_text("ColorRange"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
...
static bool v4l2_settings_changed(struct v4l2_data* data, obs_data_t* settings)
{
    ...
    if (obs_data_get_string(settings, "device_id") != NULL && data->device_id != NULL)
    {
        ...
        else
        {
            ...
            res |= (data->resolution != obs_data_get_int(settings, "resolution")) && (obs_data_get_int(settings, "resolution") != -1);
        }
