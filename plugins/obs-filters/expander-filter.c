...
static inline void process_sample(size_t idx, float* samples, float* env_buf, float* gain_db, bool is_upwcomp, float channel_gain,
                                  float threshold, float slope, float attack_gain, float inv_attack_gain, float release_gain,
                                  float inv_release_gain, float output_gain, float knee)
{
    /* --------------------------------- */
...
static inline void process_expansion(struct expander_data* cd, float** samples, uint32_t num_samples)
{
    ...
    for (size_t chan = 0; chan < cd->num_channels; chan++)
    {
        ...
        {
            process_sample(i, channel_samples, env_buf, gain_db, is_upwcomp, channel_gain, threshold, slope, attack_gain, inv_attack_gain,
                           release_gain, inv_release_gain, output_gain, knee);
        }
...
static obs_properties_t* expander_properties(void* data)
{
    ...
    if (!cd->is_upwcomp)
    {
        obs_property_t* presets = obs_properties_add_list(props, S_PRESETS, TEXT_PRESETS, OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
    ...
    obs_property_int_set_suffix(p, " ms");
    p = obs_properties_add_float_slider(props, S_OUTPUT_GAIN, TEXT_OUTPUT_GAIN, MIN_OUTPUT_GAIN_DB, MAX_OUTPUT_GAIN_DB, 0.1);
    obs_property_float_set_suffix(p, " dB");
    if (!cd->is_upwcomp)
    {
        obs_property_t* detect = obs_properties_add_list(props, S_DETECTOR, TEXT_DETECTOR, OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
