...
static bool nvafx_initialize_internal(void* data)
{
    ...
    if (!ng->handle[0])
    {
        ...
        for (size_t i = 0; i < ng->channels; i++)
        {
            ...
            if (strcmp(ng->fx, S_METHOD_NVAFX_DENOISER) != 0 && strcmp(ng->fx, S_METHOD_NVAFX_DEREVERB) != 0
            ...
            if (err != NVAFX_STATUS_SUCCESS)
            {
                do_log(LOG_ERROR, "NvAFX_SetU32(Sample Rate: %u) failed, error %i", ng->sample_rate, err);
                goto failure;
            }
            ...
            if (err != NVAFX_STATUS_SUCCESS)
            {
                do_log(LOG_ERROR, "NvAFX_SetFloat(Intensity Ratio: %f) failed, error %i", ng->intensity_ratio, err);
                goto failure;
            }
...
static void* nvafx_initialize(void* data)
{
    ...
    if (ng->use_nvafx)
    {
        ...
        NvAFX_Status err = NvAFX_GetU32(ng->handle[0], NVAFX_PARAM_NUM_INPUT_SAMPLES_PER_FRAME, &ng->num_samples_per_frame);
        if (err != NVAFX_STATUS_SUCCESS)
        {
            do_log(LOG_ERROR, "NvAFX_GetU32() failed to get the number of samples per frame, error %i", err);
            goto failure;
        }
        if (ng->num_samples_per_frame != NVAFX_FRAME_SIZE)
        {
            do_log(LOG_ERROR, "The number of samples per frame has changed from 480 (= 10 ms) ==> update code");
            goto failure;
        }
    }
...
static void noise_suppress_update(void* data, obs_data_t* s)
{
    ...
    bool nvafx_requested = strcmp(method, S_METHOD_NVAFX_DENOISER) == 0 || strcmp(method, S_METHOD_NVAFX_DEREVERB) == 0
...
bool load_nvafx(void)
{
    ...
    if (version)
    {
        ...
        {
            blog(LOG_INFO, "[noise suppress]: NVIDIA AUDIO Effects SDK is outdated. Please update both audio & video SDK.");
        }
    }
    if (!load_lib())
    {
        blog(LOG_INFO, "[noise suppress]: NVIDIA denoiser disabled, redistributable not found or could not be loaded.");
        return false;
    }
...
static inline void process_rnnoise(struct noise_suppress_data* ng)
{
    ...
    if (ng->rnn_resampler)
    {
        ...
        uint32_t out_frames;
        uint64_t ts_offset;
        audio_resampler_resample(ng->rnn_resampler, (uint8_t**)output, &out_frames, &ts_offset, (const uint8_t**)ng->copy_buffers,
                                 (uint32_t)ng->frames);

        for (size_t i = 0; i < ng->channels; i++)
        {
            for (ssize_t j = 0, k = (ssize_t)out_frames - RNNOISE_FRAME_SIZE; j < RNNOISE_FRAME_SIZE; ++j, ++k)
            {
                if (k >= 0)
                {
...
static inline void process_nvafx(struct noise_suppress_data* ng)
{
    ...
    if (nvafx_loaded && ng->use_nvafx && ng->nvafx_initialized)
    {
        ...
        if (ng->nvafx_resampler)
        {
            ...
            uint32_t out_frames;
            uint64_t ts_offset;
            audio_resampler_resample(ng->nvafx_resampler, (uint8_t**)output, &out_frames, &ts_offset, (const uint8_t**)ng->copy_buffers,
                                     (uint32_t)ng->frames);

            for (size_t i = 0; i < ng->channels; i++)
            {
                for (ssize_t j = 0, k = (ssize_t)out_frames - NVAFX_FRAME_SIZE; j < NVAFX_FRAME_SIZE; ++j, ++k)
                {
                    if (k >= 0)
                    {
        ...
        if (ng->reinit_done)
        {
            ...
            {
                NvAFX_Status err = NvAFX_Run(ng->handle[i], &ng->nvafx_segment_buffers[i], &ng->nvafx_segment_buffers[i],
                                             ng->num_samples_per_frame, ng->num_channels);
                if (err != NVAFX_STATUS_SUCCESS)
                {
                    if (err == NVAFX_STATUS_FAILED)
                    {
                        do_log(LOG_DEBUG, "NvAFX_Run() failed, error NVAFX_STATUS_FAILED.\n"
...
static bool noise_suppress_method_modified(obs_properties_t* props, obs_property_t* property, obs_data_t* settings)
{
    ...
    bool enable_intensity = strcmp(method, S_METHOD_NVAFX_DENOISER) == 0 || strcmp(method, S_METHOD_NVAFX_DEREVERB) == 0
                            || strcmp(method, S_METHOD_NVAFX_DEREVERB_DENOISER) == 0;
...
static obs_properties_t* noise_suppress_properties(void* data)
{
...
#if defined(LIBRNNOISE_ENABLED) && defined(LIBSPEEXDSP_ENABLED)
    obs_property_t* method = obs_properties_add_list(ppts, S_METHOD, TEXT_METHOD, OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
    ...
    if (ng->nvafx_enabled)
    {
        ...
        obs_property_list_add_string(method, TEXT_METHOD_NVAFX_DEREVERB_DENOISER, S_METHOD_NVAFX_DEREVERB_DENOISER);
    }
...
#ifdef LIBSPEEXDSP_ENABLED
    obs_property_t* speex_slider = obs_properties_add_int_slider(ppts, S_SUPPRESS_LEVEL, TEXT_SUPPRESS_LEVEL, SUP_MIN, SUP_MAX, 1);
