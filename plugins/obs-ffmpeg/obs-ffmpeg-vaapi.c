...
static bool vaapi_update(void* data, obs_data_t* settings, bool hevc)
{
    ...
    info("settings:\n"
         ...
         "\tb-frames:     %d\n"
         "\tffmpeg opts:  %s\n",
         device, rate_control, profile, level, qp, bitrate, maxrate, enc->context->gop_size, enc->context->width, enc->context->height,
...
static bool vaapi_encode_internal(void* data, struct encoder_frame* frame, struct encoder_packet* packet, bool* received_packet, bool hevc)
{
    struct vaapi_encoder* enc = data;
    ...
    if (got_packet && enc->packet->size)
    {
        if (enc->first_packet)
        {
            ...
            if (hevc)
            {
                obs_extract_hevc_headers(enc->packet->data, enc->packet->size, &new_packet, &size, &enc->header, &enc->header_size,
...
#endif
            {
                obs_extract_avc_headers(enc->packet->data, enc->packet->size, &new_packet, &size, &enc->header, &enc->header_size,
                                        &enc->sei, &enc->sei_size);
            }
...
static bool h264_vaapi_encode(void* data, struct encoder_frame* frame, struct encoder_packet* packet, bool* received_packet)
{
    return vaapi_encode_internal(data, frame, packet, received_packet, false);
}

#ifdef ENABLE_HEVC
static bool hevc_vaapi_encode(void* data, struct encoder_frame* frame, struct encoder_packet* packet, bool* received_packet)
{
...
static obs_properties_t* vaapi_properties_internal(bool hevc)
{
    ...
    list = obs_properties_add_list(props, "vaapi_device", obs_module_text("VAAPI.Device"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
    ...
    list = obs_properties_add_list(props, "profile", obs_module_text("Profile"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
    if (hevc)
    {
    ...
    list = obs_properties_add_list(props, "level", obs_module_text("Level"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
    ...
    list = obs_properties_add_list(props, "rate_control", obs_module_text("RateControl"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
